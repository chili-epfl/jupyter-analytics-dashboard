...
import { BACKEND_API_URL, CURRENT_NOTEBOOK_ID, setCurrentNotebookId } from '..';
...
import { APP_ID, CommandIDs, DASHBOARD_USERNAME_KEY, Selectors } from '../utils/constants';
...
class PushNotebookService implements IPushNotebookService {
    ...
    async pushCellUpdate(panelManager: PanelManager): Promise<void> {
        ...
        let notebookID = CURRENT_NOTEBOOK_ID;
        ...
        if (activeCell && pm.panel) {
            ...
            await this.pushUpdateToStudents(this.panelManager, JSON.stringify(payload));
        }
    }
...
export const pushNotebookUpdateServicePlugin: JupyterFrontEndPlugin<
...
> = {
    ...
    activate: (
        ...
        console.log(`JupyterLab extension ${APP_ID}: pushNotebookUpdate plugins activated!`);
        ...
        app.restored.then(() => {
            ...
            app.commands.addCommand(CommandIDs.pushNotebookUpdate, {
                ...
                icon: shareIcon,
                isVisible: () => service.getPanelManager().panel !== null,
                execute: () => service.pushNotebookUpdate(undefined as any)
            });
            ...
            palette.addItem({ command: CommandIDs.pushCellUpdate, category: 'Notebook' });
            palette.addItem({ command: CommandIDs.pushNotebookUpdate, category: 'Notebook' });
        });
