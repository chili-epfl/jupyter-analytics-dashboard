...
interface IUpdateStat {
  update_id: string;
  timestamp: string;
  update_now: number;
  update_later: number;
  cell_id?: string;
  detailed_actions?: IDetailedAction[];
}
...
const PendingUpdatesChart = (props: { notebookId: string }) => {
  const [stats, setStats] = useState<IUpdateStat[]>([]);
  ...
  const [visualizationType, setVisualizationType] =
    useState<VisualizationType>('histogram'); // Default to histogram for better scalability
  ...
  const calculateTimeBins = (update: IUpdateStat | undefined) => {
    if (
      !update ||
      !update.detailed_actions ||
      update.detailed_actions.length === 0
    ) {
  ...
  const calculateSummaryStats = (update: IUpdateStat | undefined) => {
    if (!update) {
      return {
        totalStudents: 0,
        responseRate: 0,
    ...
    const responseRate =
      totalStudents > 0 ? Math.round((actedCount / totalStudents) * 100) : 0;
  ...
  const summaryCards = selectedUpdate
    ? [
        ...
        },
        {
          ...
          icon: '⚡',
          color:
            summaryStats.responseRate >= 70
              ? '#4CAF50'
              : summaryStats.responseRate >= 40
                ? '#FF9800'
                : '#F44336'
        },
        {
          ...
          value:
            summaryStats.medianResponseTime > 0
  ...
  if (stats.length === 0) {
    return (
      ...
      >
        No pending updates found. Push an update to students to see statistics
        here.
      </div>
    );
  }
  ...
  return (
    ...
    >
        ...
        <>
          ...
          >
            ...
            <p style={{ margin: '4px 0' }}>
              This shows how quickly students responded after you pushed the
              update.
              <strong style={{ color: '#4CAF50' }}> Green bars</strong> = fast
              response,
              <strong style={{ color: '#4CAF50' }}> Green bars</strong> = fast
              response,
              <strong style={{ color: '#FF9800' }}> orange/yellow</strong> =
              moderate,
              <strong style={{ color: '#F44336' }}> red</strong> = slow
              response.
            </p>
            <p style={{ margin: '4px 0' }}>
              Use median response time (half responded faster, half slower) to
      ...
      {selectedUpdate &&
        selectedUpdate.detailed_actions &&
        selectedUpdate.detailed_actions.length > 0 && (
          <>
            <SectionHeader
              icon="🔍"
            ...
            {showDetailed && (
              ...
              >
                ...
                <div style={{ marginBottom: '16px' }}>
                  ...
                  >
                    {(
                      ['scatter', 'histogram', 'summary'] as VisualizationType[]
                    ).map(type => (
                      <button
                        key={type}
                ...
                <div style={{ marginBottom: '16px' }}>
                  ...
                  <div
                    style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}
                  >
                    {(Object.keys(ACTION_COLORS) as ActionType[]).map(
                      action => {
                ...
                {(() => {
                  ...
                  const filteredActions = (
                    ...
                    if (!selectedActions.has(actionType)) {
                      return false;
                    }
                    ...
                    if (timeRangeMinutes) {
                      ...
                      if (minutesDiff > timeRangeMinutes) {
                        return false;
                      }
                    }
                  ...
                  if (visualizationType === 'scatter') {
                    ...
                    const displayActions =
                      filteredActions.length > MAX_SCATTER_POINTS
                      ...
                      datasets: (Object.keys(ACTION_COLORS) as ActionType[])
                        ...
                        .map(action => ({
                          ...
                          data: displayActions
                            ...
                            .map((a, idx) => ({
                              x:
                                (new Date(a.timestamp).getTime() - pushTime) /
                                1000 /
                                60,
                              y: idx
                            })),
                    ...
                    return (
                      <>
                        {isLimited && (
                          ...
                          >
                            ℹ️ Showing most recent {MAX_SCATTER_POINTS} of{' '}
                            {filteredActions.length} actions for better
                            performance. Use histogram for full overview.
                          </div>
                        )}
                        <div style={{ height: '300px' }}>
                          <ChartContainer
                        ...
                        .map(action => {
                          const counts = bins.map(binStart => {
                            return filteredActions.filter(a => {
                              if (a.action.toLowerCase() !== action) {
                                return false;
                              }
                              const mins =
                                (new Date(a.timestamp).getTime() - pushTime) /
                  ...
                  const tableDisplayActions =
                    filteredActions.length > MAX_TABLE_ROWS
                      ? filteredActions.slice(-MAX_TABLE_ROWS)
                      : filteredActions;
                  const tableIsLimited =
                    filteredActions.length > MAX_TABLE_ROWS;
                  ...
                  return (
                    <>
                      {tableIsLimited && (
                        ...
                        >
                          ℹ️ Showing most recent {MAX_TABLE_ROWS} of{' '}
                          {filteredActions.length} actions. Use histogram for
                          full overview.
                        </div>
                      )}
                      <div
                                    ...
                                    style={{
                                      backgroundColor:
                                        idx % 2 === 0
                                          ...
                                          : 'var(--jp-layout-color2)',
                                      borderBottom:
                                        '1px solid var(--jp-border-color1)'
                                    }}
                                  >
                                    <td style={{ padding: '10px' }}>
                                      ...
                                      >
                                        {ACTION_COLORS[actionType]?.label ||
                                          action.action}
                                      </span>
                                    </td>
                                    <td
